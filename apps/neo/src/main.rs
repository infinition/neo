...
#[command(
    name = "neo",
    version,
    about = "Neo — Zero-copy VRAM video processing for the AI era",
    long_about = r#"
  ███╗   ██╗███████╗ ██████╗       ███████╗███████╗
  ████╗  ██║██╔════╝██╔═══██╗      ██╔════╝██╔════╝
  ██╔██╗ ██║█████╗  ██║   ██║█████╗█████╗  █████╗
...
enum Commands {
    ...
    /// Stream an H.264 file over TCP — zero-copy NVDEC→NVENC sender
    Send(commands::stream::send::Args),

    /// Receive, decode and display a TCP video stream with latency metrics
    Recv(commands::stream::recv::Args),

    /// Capture the desktop (DXGI), encode with NVENC, stream over TCP
    Desktop(commands::desktop::Args),

    /// N×N video wall — one NVDEC stream per tile, a different GPU filter each
    Mosaic(commands::mosaic::Args),

    /// Zero-copy RIFE frame interpolation streamer (2× FPS)
    Rife(commands::rife::Args),

    /// Hot-swappable ONNX filter streamer — drop .onnx files into a watched dir
    FilterLive(commands::filter_live::Args),

    /// Zero-copy ONNX inference micro-bench + practical video runner
    InferBench(commands::infer_bench::Args),
}
...
fn main() {
    ...
    match cli.command {
        ...
        Commands::Send(args) => {
            if let Err(e) = commands::stream::send::run(args) {
                eprintln!("  Send failed: {e}");
                std::process::exit(1);
            }
        }
        Commands::Recv(args) => commands::stream::recv::run(args),
        Commands::Desktop(args) => {
            if let Err(e) = commands::desktop::run(args) {
        ...
        Commands::Rife(args) => {
            if let Err(e) = commands::rife::run(args) {
                eprintln!("  RIFE failed: {e}");
                std::process::exit(1);
            }
        }
        Commands::FilterLive(args) => {
            if let Err(e) = commands::filter_live::run(args) {
                eprintln!("  Filter-live failed: {e}");
                std::process::exit(1);
            }
        }
        Commands::InferBench(args) => {
            if let Err(e) = commands::infer_bench::run(args) {
