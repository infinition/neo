...
# configurations, repeats each config N times, and emits a CSV summarised in
...
#   - source clips at assets/videos/src_1080p.{h264,mp4} and src_2160p.{h264,mp4}
#
...
RUNS=${RUNS:-5}
RESULTS=benchmarks/results/results.csv
...
[[ -x "$NEO" ]] || die "neo.exe not found at $NEO (cargo build --release -p neo-app)"
...
warmup() {
    ...
    run_neo zerocopy warmup assets/videos/src_1080p.h264 0 >/dev/null 2>&1 || true
    run_ffmpeg_nvdec_nvenc warmup assets/videos/src_1080p.mp4 0 >/dev/null 2>&1 || true
    run_ffmpeg_cpu warmup assets/videos/src_1080p.mp4 0 >/dev/null 2>&1 || true
...
for res in 1080p 2160p; do
    echo
    echo "→ resolution: ${res}"
    h264=assets/videos/src_${res}.h264
    mp4=assets/videos/src_${res}.mp4
