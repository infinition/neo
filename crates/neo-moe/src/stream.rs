...
impl ExpertStream {
    ...
    pub fn init(config: MoeConfig) -> Result<Self> {
        ...
        let device     = CudaContext::new(config.cuda_device as usize)?;
...
unsafe fn alloc_pinned(size: usize) -> (*mut u8, bool) {
    ...
    // SAFETY: cuMemAllocHost_v2 allocates page-locked memory; the caller
    // must free it via cuMemFreeHost.
    let r = unsafe { cuda_sys::cuMemAllocHost_v2(&mut ptr, size) };
...
unsafe fn h2d_copy(dst_ptr: *mut u8, src: &[u8]) -> bool {
    // SAFETY: dst_ptr points to a valid VRAM allocation of sufficient size;
    // src is a valid host buffer. The CUDA runtime serialises this.
    let r = unsafe {
...
fn io_worker_uring(
    ...
    if let Err(e) = device.bind_to_thread() {
        error!("Worker {thread_id}: bind_to_thread failed: {e:?}");
        return;
    }
    ...
    if !staging_ptr.is_null() {
        unsafe { cuda_sys::cuMemFreeHost(staging_ptr as *mut std::ffi::c_void) };
    }
...
fn io_worker_mmap(
    ...
    if let Err(e) = device.bind_to_thread() {
        eprintln!("[neo-moe-rs] worker {thread_id} failed to bind CUDA context: {e:?}");
        error!("Worker {thread_id}: bind_to_thread failed: {e:?}");
        return;
    }

    eprintln!("[neo-moe-rs] mmap worker {thread_id} starting");
    debug!("mmap worker {thread_id} starting");

    let file = match File::open(path) {
        Ok(f) => f,
        Err(e) => {
            eprintln!("[neo-moe-rs] worker {thread_id} open failed: {e}");
            error!("Worker {thread_id}: open failed: {e}");
            return;
        }
    };
    let mmap = match unsafe { MmapOptions::new().map(&file) } {
        Ok(m) => Arc::new(m),
        Err(e) => {
            eprintln!("[neo-moe-rs] worker {thread_id} mmap failed: {e}");
            error!("Worker {thread_id}: mmap failed: {e}");
            return;
        }
    };
    ...
    let (staging_ptr, is_pinned) = unsafe { alloc_pinned(max_expert) };
    if !is_pinned {
        eprintln!("[neo-moe-rs] worker {thread_id} pinned alloc failed; using pageable staging");
        warn!("Worker {thread_id}: cuMemAllocHost failed, using pageable staging");
    }

    for req in rx {
        let id = req.id;
        eprintln!("[neo-moe-rs] worker {thread_id} request layer={} expert={}", id.layer, id.expert);
        trace!("Worker {thread_id}: mmap load {:?}", id);

        let weights = match experts.get(&id) {
            Some(w) => w,
            None => {
                eprintln!("[neo-moe-rs] worker {thread_id} expert missing layer={} expert={}", id.layer, id.expert);
                error!("Worker {thread_id}: expert {id:?} not in map");
                continue;
            }
        };
        ...
        let (slot_idx, _) = {
            ...
            match p.acquire() {
                Ok(r) => r,
                Err(e) => {
                    eprintln!("[neo-moe-rs] worker {thread_id} acquire failed: {e}");
                    error!("Worker {thread_id}: acquire: {e}");
                    continue;
                }
            }
        };
        ...
        // staging → VRAM.
        {
            ...
            let dst_ptr = p.device_ptr(slot_idx);
            if !unsafe { h2d_copy(dst_ptr, &staging[..total]) } {
                eprintln!("[neo-moe-rs] worker {thread_id} H2D failed layer={} expert={} bytes={} slot={}", id.layer, id.expert, total, slot_idx);
        ...
        pool.lock().unwrap().mark_resident(slot_idx, id);
        eprintln!("[neo-moe-rs] worker {thread_id} ready layer={} expert={} slot={} bytes={}", id.layer, id.expert, slot_idx, total);
