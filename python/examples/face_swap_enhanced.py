...
def paste_enhanced(target_img, bgr_fake_enhanced, M):
    M_inv = cv2.invertAffineTransform(M)
    M_inv_enhanced = M_inv.copy()
    M_inv_enhanced[:, :2] /= 4.0
    mask = np.zeros((512, 512), dtype=np.float32)
    cv2.circle(mask, (256, 256), 220, 1.0, -1)
    mask = cv2.GaussianBlur(mask, (51, 51), 0)
    bgr_fake_warped = cv2.warpAffine(bgr_fake_enhanced, M_inv_enhanced, (target_img.shape[1], target_img.shape[0]), borderValue=0.0)
    mask_warped = cv2.warpAffine(mask, M_inv_enhanced, (target_img.shape[1], target_img.shape[0]), borderValue=0.0)
    mask_warped = mask_warped[:, :, np.newaxis]
    fake_merged = mask_warped * bgr_fake_warped + (1 - mask_warped) * target_img.astype(np.float32)
...
def main():
    ...
    print("[init] RealESRGANv3 (Super Résolution Face x4) ...")
    realesr_sess = ort.InferenceSession("assets/models/RealESRGANv2/realesr-animevideov3.onnx", providers=["CUDAExecutionProvider"])
    realesr_input = realesr_sess.get_inputs()[0].name
    ...
    def worker():
        ...
        while not shared["stop"]:
            ...
            for f in last_faces:
                try:
                    bgr_fake, M = swapper.get(bgr, f, source_face, paste_back=False)
                    rgb = bgr_fake[:, :, ::-1]
                    tensor = np.transpose(rgb, (2, 0, 1)).astype(np.float32) / 255.0
                    tensor = np.expand_dims(tensor, 0)
                    out = realesr_sess.run(None, {realesr_input: tensor})[0]
                    out = np.squeeze(out)
                    out = np.clip(out, 0, 1) * 255.0
                    bgr_fake_enhanced = np.transpose(out, (1, 2, 0)).astype(np.uint8)[:, :, ::-1]
                    bgr = paste_enhanced(bgr, bgr_fake_enhanced, M)
