...
// ============================================================================
// Neo-FFmpeg patch — dynamic loading
//
// The original bindgen output here was an `extern "C" { ... }` block declaring
// every NVENC entry point as a static link symbol against `nvEncodeAPI.lib`.
// That hard-linked the crate to the NVIDIA Video Codec SDK at build time.
//
// In practice the safe layer in this crate only ever calls TWO of those
// symbols (`NvEncodeAPICreateInstance` and `NvEncodeAPIGetMaxSupportedVersion`)
// — every other entry point is reached via the function-pointer table that
// `NvEncodeAPICreateInstance` populates. So we replace the extern block with
// runtime-loaded shims that pull the two symbols out of `nvEncodeAPI64.dll`
// (Windows) / `libnvidia-encode.so.1` (Linux) using `libloading`.
//
// Result: this crate compiles with **only** an NVIDIA driver present at run
// time — no SDK install, no `.lib` import library required.
// ============================================================================

mod nvenc_loader {
    use super::{NVENCSTATUS, NV_ENCODE_API_FUNCTION_LIST};
    use std::sync::OnceLock;

    pub(super) struct NvEncLib {
        _lib: libloading::Library,
        pub create_instance:
            unsafe extern "C" fn(*mut NV_ENCODE_API_FUNCTION_LIST) -> NVENCSTATUS,
        pub get_max_supported_version: unsafe extern "C" fn(*mut u32) -> NVENCSTATUS,
    }

    static NVENC: OnceLock<Result<NvEncLib, String>> = OnceLock::new();

    #[cfg(windows)]
    const LIB_NAMES: &[&str] = &["nvEncodeAPI64.dll", "nvEncodeAPI.dll"];
    #[cfg(unix)]
    const LIB_NAMES: &[&str] = &["libnvidia-encode.so.1", "libnvidia-encode.so"];

    fn load_inner() -> Result<NvEncLib, String> {
        ...
        for name in LIB_NAMES {
            match unsafe { libloading::Library::new(name) } {
                Ok(lib) => unsafe {
                    let create_instance = lib
                        .get::<unsafe extern "C" fn(
                            *mut NV_ENCODE_API_FUNCTION_LIST,
                        ) -> NVENCSTATUS>(
                            b"NvEncodeAPICreateInstance\0",
                        )
                        .map_err(|e| {
                            format!("missing NvEncodeAPICreateInstance in {name}: {e}")
                        })?;
                    let get_max_supported_version = lib
                        .get::<unsafe extern "C" fn(*mut u32) -> NVENCSTATUS>(
                            b"NvEncodeAPIGetMaxSupportedVersion\0",
                        )
                        .map_err(|e| {
                            format!("missing NvEncodeAPIGetMaxSupportedVersion in {name}: {e}")
                        })?;
                    let create_instance = *create_instance.into_raw();
                    let get_max_supported_version = *get_max_supported_version.into_raw();
                    return Ok(NvEncLib {
                        _lib: lib,
                        create_instance,
                ...
                Err(e) => last_err = format!("could not load {name}: {e}"),
            }
        }
        Err(last_err)
    }

    pub(super) fn get() -> Result<&'static NvEncLib, String> {
        NVENC.get_or_init(load_inner).as_ref().map_err(|e| e.clone())
    }
}

/// Dynamic shim for `NvEncodeAPICreateInstance`. Loads `nvEncodeAPI64.dll`
/// (or platform equivalent) on first call.
///
/// # Safety
/// `function_list` must point to a valid `NV_ENCODE_API_FUNCTION_LIST` whose
/// `version` field has been set to `NV_ENCODE_API_FUNCTION_LIST_VER`.
#[must_use]
pub unsafe fn NvEncodeAPICreateInstance(
    function_list: *mut NV_ENCODE_API_FUNCTION_LIST,
) -> NVENCSTATUS {
    match nvenc_loader::get() {
        Ok(lib) => (lib.create_instance)(function_list),
        // 15 == NV_ENC_ERR_NO_ENCODE_DEVICE — closest match for "library not present".
        Err(_) => NVENCSTATUS::NV_ENC_ERR_NO_ENCODE_DEVICE,
    }
}

/// Dynamic shim for `NvEncodeAPIGetMaxSupportedVersion`.
///
/// # Safety
/// `version` must be a valid pointer to a `u32`.
#[must_use]
pub unsafe fn NvEncodeAPIGetMaxSupportedVersion(version: *mut u32) -> NVENCSTATUS {
    match nvenc_loader::get() {
        Ok(lib) => (lib.get_max_supported_version)(version),
        Err(_) => NVENCSTATUS::NV_ENC_ERR_NO_ENCODE_DEVICE,
    }
}
